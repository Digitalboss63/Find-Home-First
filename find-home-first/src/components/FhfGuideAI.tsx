...
interface NaturalAnswer {
  answer: string;
  source: "ai" | "workflow";
  action?: GuideProjectContext["nextAction"];
}
...
function classifyProjectQuestion(query: string): ProjectQuestion | null {
  ...
  if (normalized.includes("block") || normalized.includes("stuck")) return "blocker";
...
function ProjectAnswer({
  ...
  if (mode === "blocker") {
    return context.blocker ? (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
        ...
        {context.blocker.reason && <p className="mt-2 text-sm leading-5 text-amber-900">{context.blocker.reason}</p>}
        <p className="mt-3 text-sm font-medium text-amber-950">Recommended action: {context.nextAction.label}</p>
      </div>
    ) : (
  ...
  return (
    <div className="rounded-xl border border-blue-200 bg-blue-50/70 p-4">
      ...
      <h3 className="mt-1 text-base font-semibold text-blue-950">{context.project?.name ?? "No active project"}</h3>
      <p className="mt-2 text-sm leading-5 text-blue-950">{context.summary}</p>
      {context.project && (
        <div className="mt-3 flex flex-wrap gap-2 text-xs">
          {context.stageLabel && <span className="rounded-full bg-white px-2.5 py-1 font-medium text-blue-900">Stage: {context.stageLabel}</span>}
          <span className="rounded-full bg-white px-2.5 py-1 font-medium text-blue-900">Saved properties: {context.savedPropertyCount}</span>
          <span className="rounded-full bg-white px-2.5 py-1 font-medium text-blue-900">Open tasks: {context.openTasks.length}</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="rounded-xl border border-violet-200 bg-violet-50/70 p-4">
      <p className="text-xs font-semibold uppercase tracking-wide text-violet-700">
        {result.source === "ai" ? "FHF smart guidance" : "FHF workflow guidance"}
      </p>
      <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-violet-950">{result.answer}</p>
      {result.action && (
        <Link
...
export default function FhfGuideAI() {
  ...
  const [naturalAnswer, setNaturalAnswer] = useState<NaturalAnswer | null>(null);
  ...
  const loadProjectContext = async () => {
    ...
    try {
      ...
      console.warn("[fhf-guide] project context unavailable", error instanceof Error ? error.message : String(error));
  ...
  const askNaturalQuestion = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const question = query.trim();
    if (question.length < 2) return;

    const directQuestion = classifyProjectQuestion(question);
    if (directQuestion) {
      setNaturalAnswer(null);
      setAnswerState("idle");
      setProjectQuestion(directQuestion);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Guide answer failed with ${response.status}`);
      const payload = (await response.json()) as NaturalAnswer;
      if (!payload.answer) throw new Error("Guide answer was empty");
      setNaturalAnswer(payload);
      setAnswerState("ready");
    } catch (error) {
      console.warn("[fhf-guide] natural answer unavailable", error instanceof Error ? error.message : String(error));
  ...
  return (
    <>
      ...
      {open && (
        <>
          <button type="button" className="fixed inset-0 z-40 cursor-default bg-black/35" aria-label="Close FHF Guide" onClick={close} />
          <section
            role="dialog"
            aria-modal="true"
          ...
          >
            <header className="border-b border-slate-200 px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--color-primary)" }}>Find Home First</p>
                  <h2 id="fhf-guide-title" className="mt-1 text-xl font-bold text-slate-950">FHF Guide</h2>
                  <p className="mt-1 text-sm text-slate-600">Ask a normal question about the app or your current workflow.</p>
                </div>
                <button ref={closeButtonRef} type="button" onClick={close} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900" aria-label="Close FHF Guide">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5">
              {selectedTopic ? (
                ...
                <div className="space-y-5">
                  <form onSubmit={askNaturalQuestion} className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="Example: I found a house. What do I do with it?"
                        className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-950 outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200"
                      />
                      <button
                        type="submit"
                        disabled={answerState === "loading" || query.trim().length < 2}
                    ...
                    <p className="text-[11px] leading-4 text-slate-400">Search results appear below as you type. Press Ask for a direct answer.</p>
                  </form>

                  {!query.trim() && (
                    <div className="flex flex-wrap gap-2" aria-label="Project guidance questions">
                      <button type="button" onClick={() => chooseProjectQuestion("next")} className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">What do I do now?</button>
                      <button type="button" onClick={() => chooseProjectQuestion("blocker")} className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">What&apos;s blocking me?</button>
                      <button type="button" onClick={() => chooseProjectQuestion("missing")} className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">What&apos;s missing?</button>
                    </div>
                  )}

                  {answerState === "loading" && (
                    <div className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950">Thinking through the FHF workflow…</div>
                  )}
                  {answerState === "ready" && naturalAnswer && <NaturalAnswerCard result={naturalAnswer} onClose={close} />}
                  {answerState === "error" && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm leading-5 text-amber-950">I could not generate a direct answer right now. The project overview and Help Center guidance are still available below.</div>
                  )}

                  {contextState === "loading" && answerState === "idle" && (
                    <div className="rounded-xl border border-blue-100 bg-blue-50/70 p-4 text-sm text-blue-950">Reading your active project…</div>
                  )}

                  {contextState === "ready" && projectContext && answerState === "idle" && (
                    <ProjectAnswer context={projectContext} mode={activeQuestion} onClose={close} />
                  )}

                  {contextState === "error" && answerState === "idle" && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm leading-5 text-amber-950">I can still explain Find Home First features, but I could not read the active project right now.</div>
                  )}

                  {!query.trim() && activeQuestion === "overview" && answerState === "idle" && (
                    <div className="rounded-xl border border-blue-100 bg-blue-50/70 p-3">
                      <p className="text-xs font-semibold uppercase tracking-wide text-blue-700">Help for this page</p>
                      <p className="mt-1 text-sm text-blue-950">I found {contextualTopics.length} topic{contextualTopics.length === 1 ? "" : "s"} related to where you are now.</p>
                    </div>
                  )}
                  ...
                  {!detectedProjectQuestion && (
                    <div className="space-y-2">
                      {results.length > 0 ? (
                        results.map((topic) => (
                          <button key={topic.id} type="button" onClick={() => setSelectedTopic(topic)} className="w-full rounded-xl border border-slate-200 p-4 text-left transition hover:border-slate-300 hover:bg-slate-50">
                        ...
                        <div className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-600">No matching help article. Press <strong>Ask</strong> and FHF Guide will answer using the current workflow and the closest available guidance.</div>
                      )}
                    </div>
                  )}
                </div>
            ...
            <footer className="border-t border-slate-200 px-5 py-4">
              <div className="flex items-center justify-between gap-3">
                {selectedTopic ? (
                  <button type="button" onClick={() => setSelectedTopic(null)} className="text-sm font-semibold text-slate-700 hover:text-slate-950">← Back to topics</button>
                ) : (
                  <button type="button" onClick={() => chooseProjectQuestion("overview")} className="text-xs font-semibold text-slate-600 hover:text-slate-950">Active project overview</button>
                )}
                <Link href="/help" onClick={close} className="text-sm font-semibold" style={{ color: "var(--color-primary)" }}>Open Help Center</Link>
              </div>
              <p className="mt-2 text-[11px] leading-4 text-slate-400">{HELP_TOPICS.length} help topics. Smart answers use privacy-minimized workflow context; resident names, contact details, property addresses, and task text are not sent to the model.</p>
            </footer>
          </section>
        </>
      )}
